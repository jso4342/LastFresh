...
public class FileCheck {
    ...
    public void checkFiles() throws Exception{
        ...
        List<ProductDTO> fileList = ownerProductDAO.getOldFiles();
        log.info("ProductList " + fileList.toString());
        //모델 객체에 있는 uploadPath만 가져오기 위해 map을 사용한다.
        List<Path> fileListPaths = fileList.stream()
                .map(file -> Paths.get("C:/upload", file.getSellProductImageUploadPath(), file.getSellProductThumbnail()))
                .collect(Collectors.toList());

        //이미지 파일은 썸네일이 있기 때문에 썸네일 경로도 추가해준다.
        fileList.stream()
                .map(file -> Paths.get("C:/upload", file.getSellProductImageUploadPath(), file.getSellProductImage()))
